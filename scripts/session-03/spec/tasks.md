...
- [ ] **5.4** 基本編の手順を書く → 🚧 **骨格のみ**（実画面の確認が必要／`next-actions.md` C-1・C-2）
...
- [ ] **5.5** 「デバイス側での確認」セクションを独立した節として書く → 🚧 **骨格のみ**（確認コマンド一覧と `free` の定義差の説明は記述済み。実測値が未記入／`next-actions.md` B-4）
...
| ファイル | 状態 | 備考 |
| --- | --- | --- |
| `docs/session-03/handson.md` | 🚧 骨格作成済み | ゴール / 進め方 / 所要時間 / 学習内容 / 構成図は記述完了。実測依存の 6 セクションが `<!-- TODO -->` |
