...
| メモリ負荷 | 目標使用率から現在使用量を差し引いた追加バイト数を算出し、`bytearray` を確保して保持。ページを実際に触るため書き込みを行う（遅延割り当て回避） |
| 上限ガード（R2-6） | 確保後のメモリ総使用量が**総容量の 85% を超えない**よう追加確保量をクランプし、警告を表示。CPU の `--target` は 1〜100 に検証 |
...
D-5 によりクロススタック参照がないため、削除順序に失敗しても個別にリトライできる。CloudWatch Metrics は削除 API が存在せず、2 週間データポイントがなければコンソール等の一覧から非表示になり、データ自体は解像度を下げながら最長 15 か月保持された後に期限切れとなる。その旨を出力で明示する（参加者が「消え残っている」と誤解しないため）。
...
合計で参加者 1 名あたり十数円規模。**カスタムメトリクスと Alarm は月額課金**であるため、後片付けで Alarm を削除することの意味を手順書で触れる（メトリクス自体は削除 API がなく、送信停止後も解像度を下げながら最長 15 か月保持される）。
