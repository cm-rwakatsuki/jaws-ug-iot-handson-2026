...
- [x] 確認メールを受信し承認、SNS で `確認済み` を確認（R5-2）… 2026-08-14→ **未実施**
- [x] CFN が承認を待たずに `CREATE_COMPLETE` になることを確認（K-6 の裏付け）… 確認済み→ **未実施**
- [x] `データ不足` → `OK` → `ALARM` → `OK` の遷移を確認（R5-4）… `describe-alarm-history` で裏取り→ **未実施**
- [x] 5 分以内にメール受信（R5-5）… **実測 約 2 分**→ **未実施**
- [x] 復旧通知を受信（R5-7）… `ALARM → OK` で受信→ **未実施**
- [ ] 送信停止中に誤発火しない（`TreatMissingData: notBreaching`）（R5-6）→ **未実施**
- [ ] しきい値・期間の既定値を実測に基づいて確定（Q2）→ **未実施**（暫定値のまま）
- [~] スタック再作成時に再承認が必要であることを確認（K-6）… ⏭ **検証不要と判断**（既知挙動として手順書に文書化済み）→ **未実施**
...
**実際の結果**（2026-08-14 01:5x〜02:0x JST。コンソールで実施）

| 確認項目 | 結果 |
| --- | --- |
| 確認メール到着までの時間 | ✅ **即時**（すぐ届いた） |
| 迷惑メールフォルダに入ったか | ✅ **入らなかった**（個人 Gmail を使用） |
| 承認前のスタック状態 | ✅ 承認を待たずに `CREATE_COMPLETE`（**K-6 の裏付け**） |
| 承認後のステータス | ✅ `確認済み`（Confirmed）。`list-subscriptions-by-topic` で ARN も確認 |
...
**実際の結果**（2026-08-14 実施。`describe-alarm-history` で裏取り）

| 時刻 (JST) | 遷移 | メール |
| --- | --- | --- |
| 01:59:58 | `INSUFFICIENT_DATA` → `OK` | ✅ 届く（下記の発見 1） |
| 02:03:58 | `OK` → **`ALARM`** | ✅ 届く（本命。**約 2 分**で受信） |
| 02:06:58 | `ALARM` → `OK` | ✅ **復旧通知が届く**（R5-7） |

| 確認項目 | 結果 |
| --- | --- |
| 状態遷移（R5-4） | ✅ `データ不足` → `OK` → `アラーム状態` → `OK` |
| メール受信（R5-5：5 分以内） | ✅ **約 2 分**で受信 |
| 復旧通知（R5-7） | ✅ 受信。`EnableOkNotification: true` が機能 |
| メール本文の情報量 | ✅ メトリクス名・実測値・閾値・`Period`・`Statistic`・`TreatMissingData` がすべて記載 |
| `TreatMissingData` の反映 | ✅ メール本文に `notBreaching` と明記されている（R5-6） |

**判定**: **Pass**

**🔍 発見 1：負荷をかける前に 1 通目のメールが届く**

`EnableOkNotification: true` により `OKActions` が設定されているため、
**アラーム作成直後の `INSUFFICIENT_DATA` → `OK` 遷移でもメールが飛ぶ**。
参加者は「まだ何もしていないのにメールが来た」と戸惑う可能性がある。

| 項目 | 内容 |
| --- | --- |
| 対処 | 手順書に「1 通目は正常。**本命は 2 通目の `OK -> ALARM`**」と明記 |
| 設計変更するか | **しない**。復旧通知（R5-7）は「通知経路が双方向に機能する」ことを示す教材価値があるため `true` を維持 |

**🔴 発見 2（F-12）：サブスクリプションが意図せず解除されることがある**

検証中に SNS から次のメールが届き、**通知が止まる**事象が発生した。

```
Your subscription to the topic below has been deactivated: ...
If this was in error or you wish to resubscribe, click or visit the link below: Resubscribe
```

| 項目 | 内容 |
| --- | --- |
| 状況 | 検証者は自分で解除操作をしていない |
| 考えられる原因 | **SNS のメール末尾の解除リンクは、クリックした時点で確認なしに解除される**仕様。メールクライアントやセキュリティ製品による**リンクの先読み（プリフェッチ）**、プレビュー表示、転送などで辿られると解除されうる |
| 確認した現在の状態 | `list-subscriptions-by-topic` では `確認済み`（ARN あり）。解除が一時的だったか、「Resubscribe」で復帰したかは切り分けできていない |
| 参加者への影響 | **アラームは発火しているのに通知が来ない**状態になり、原因の切り分けが難しい（K-6 と症状が同じで紛らわしい） |
| 対処 | 手順書に「通知が届かなくなったら」の節を新設。解除メールの見た目・考えられるきっかけ・復旧手順（Resubscribe → 再承認 → `確認済み` 確認）・予防策（メール内のリンクを不要にクリックしない）を記載。ハマりポイント表にも追加 |
| 残る不明点 | **再現条件が特定できていない**。当日 20 人規模で何人に起きるか未知。リハーサル（5.10）で再発するか観察する |

**証跡**: `evidence/m4/4.4-alarm-history.txt`（スクリーンショットは未取得）
...
| **2026-08-14** | **connpass URL の確定（タスク 5.9 完了）** | `README.md`、`docs/session-03/handson.md`、`tasks.md` | 第3回のイベントページ URL を反映（`https://jawsug-iot.connpass.com/event/402082/`）。これで **5.9 は完了**。手順書の運営向けメモからも該当の確認項目を削除 |
| **2026-08-14** | **タスク 4.6 は検証不要と判断** | `tasks.md`、`verification-log.md` | 「スタック再作成時に再承認が必要」の実地検証は**不要と判断**（オーナー判断）。K-6 の既知挙動であり、手順書のハマりポイント表に「スタックを作り直した → 再作成すると再承認が必要。前回承認済みでも届かない」として**文書化済み**のため。実地検証のために SNS サブスクリプションを作り直すコストに見合わない |
| **2026-08-14** | **所要時間の見立て（タスク 5.10 の一部）** | （記録のみ） | ウォークスルーを通した実感として **「スムーズに進めれば 60 分ほどで完了できる」**とオーナーが確認（NFR-4 の 90 分以内は満たす見込み）。⚠️ ただしこれは**私との対話・修正を挟みながらの作業であり、独立した通し実行の計測ではない**。パート別の内訳も未計測。正式なリハーサルとしては未完了として扱う |
| **2026-08-14** | **方針変更：スクリーンショットは挿入しない** | `docs/session-03/handson.md` | 当初は `docs/session-03/images/` に画面キャプチャを置く想定だったが、**挿入しない方針に変更**（オーナー判断）。理由：コンソールの画面表記は頻繁に変わり（実際に本検証中も CloudWatch の刷新に遭遇）、画像は追従できずかえって誤解を生む。文章で画面名・遷移を記述する方式を採る。副次的に、アカウント ID の写り込みリスクも無くなった |
| **2026-08-14** | **Q2 の確定（タスク 4.5）** | `tasks.md` | アラームのしきい値・評価期間を**既定値のまま確定**（しきい値 80% / 期間 60 秒 / 評価回数 1 / 負荷の推奨継続時間 180 秒）。根拠：平常時 1.44%・負荷時 90.07% で閾値 80% を大きく挟み、発火まで実測約 2 分。誤発火の余地がない。⚠️ ただし「送信停止中に誤発火しないこと」は狙って検証していない（設定は入っており、実際の誤発火も観測されず） |
| **2026-08-14** | **README の更新（タスク 5.9）** | `README.md` | 第3回の行を手順書へのリンクに変更。ディレクトリ構成図に session-03 配下（docs 3 / scripts 9 / cfn 3）を追記。開発者向けに `pytest` / `cfn-lint` の実行方法と spec へのリンクを追加。記載した全 16 パスの実在を検証済み。connpass URL は未確定のため `T.B.D.` のまま |
| **2026-08-14** | **`teardown.sh` の 3 スタック削除検証（タスク 2.10 / 5.8）** | `scripts/session-03/teardown.sh`、`docs/session-03/handson.md` | 3 スタック（Basic / Course1 / Course2）が存在する状態で実行し、**全リソースの削除を独立検証**（スタック 3 本・Thing・ポリシー・ルール 2 本・証明書・アラーム・SNS トピック）。**CloudWatch アラームが 0 件になり月額課金が停止**。F-3e の修正が効き、削除対象一覧にスタック名が正しく表示された（3 スタック構成でも文字化けなし） |
| **2026-08-14** | **中断→再実行の挙動を CloudTrail で確認（重要）** | （記録のみ） | 実行中に「証明書が見つからない」「alarm スタックが存在しない」と表示され、想定と食い違った。**CloudTrail で時系列を再構成**して原因を特定：オーナーが先に実行したものを途中で中断していた。`02:52:13 DetachThingPrincipal` → `02:52:14 DetachPolicy` / `UpdateCertificate(INACTIVE)` → `02:52:15 DeleteCertificate` → `02:52:16 DeleteStack(alarm)` の 4 ステップが設計どおり実行済みで、**証明書は正しく削除され孤児は発生していなかった**。その後 `02:53:39` からの再実行が lambda / basic スタックと Thing を削除して完了。→ **`teardown.sh` は中断後の再実行で続きから完了できる（冪等）ことを実証**。当日「途中で止まった」参加者が出ても再実行で片付く |
| **2026-08-14** | **上記から派生した UX 修正（F-3g）** | `scripts/session-03/teardown.sh`、`docs/session-03/handson.md` | 再実行時に「証明書が見つかりません → 孤児が残っているか確認してください」という**警告が出るが、実際は正しく削除済みだった**。F-3d 対策の文言が「すでに削除済み」と「発行したが未アタッチ」を区別できていなかった。**スクリプトには判断材料がない**ため、断定をやめて両方の可能性を示す中立な文言（`ℹ️`）に変更。実際に紐付きのない証明書を検索する案は、他プロジェクトの証明書（このアカウントに 9 枚）を誤検出するため採らなかった。手順書にも「再実行すれば続きから片付く」旨を追記 |
...
| 2026-08-14 | 同上（対応完了） | git 履歴（全 34 コミット） | オーナー判断により**案 B（履歴書き換え）を実施**。①`git bundle --all` でバックアップ（613K・リポジトリ外）②`git filter-repo --replace-text` で 3 コミット内の文字列を置換 ③ローカル・リモート全 ref で検出ゼロを検証 ④`--force-with-lease` で `feature/session-03` のみ force push ⑤`reflog expire` ＋ `gc --prune=now` で旧オブジェクトを削除。**main / session-02 / fix-... は内容一致を確認のうえ push せず**、ローカルを origin に合わせて影響を最小化。残作業：GitHub 側で旧 SHA の URL が 404 になっているかの目視確認 |
