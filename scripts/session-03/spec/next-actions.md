...
| **F-13** | `scripts/session-03/metrics_publisher.py` | **実 IoT エンドポイントがコミットされ、公開リポジトリに push されていた**。手順書が「ファイルの設定値を書き換える」方式のため起きた | **高** | ✅ **解消**：環境変数対応 ＋ 検出テスト 7 件 ＋ **`git filter-repo` で履歴から除去し force push 済み**（バックアップ取得済み） |
| **F-3g** | `teardown.sh` | 再実行時に「証明書が見つかりません → 孤児が残っているか確認」と**警告が出るが実際は正しく削除済み**だった。「削除済み」と「未アタッチ」を区別できていなかった | 中 | ✅ **解消**：断定をやめ中立な文言（`ℹ️`）に変更。手順書にも「再実行すれば続きから片付く」旨を追記 |
