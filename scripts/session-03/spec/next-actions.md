...
| F-2 | `load_gen.py` | duty cycle がベースライン負荷を考慮しない（B-3 参照） | **高** | 🔴 未対応（実機実測後に判断） |
...
| F-4 | `show_metrics.sh` | 一度も実行していない（`/proc` 依存）。`awk '/^Cached:/'` が `SwapCached:` を拾わないか、CPU 計算が破綻しないかを実機で確認（B-2） | 中 | 🔴 未対応（実機必須） |
