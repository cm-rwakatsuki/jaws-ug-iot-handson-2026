...
> ## � 運営向けメモ（公開前にこのブロックを削除する）
>
> **全セクションを実機と実 AWS 環境で通し確認済みです**（2026-08-13〜14）。
> 画面名・出力例・所要時間はすべて実測値に置き換えてあります。
>
> | セクション | 状態 |
> | --- | --- |
> | ゴール / 進め方 / 所要時間 / 学習内容 / 構成図 | ✅ 確定 |
> | AWS 側設定 | ✅ コンソールで実地確認済み |
> | 実装（Raspberry Pi） | ✅ 実機で確認済み（出力例は実測値） |
> | 動作確認 / デバイス側での確認 | ✅ 実測値を反映（3 値の突き合わせも確認） |
> | Advanced Course1（Lambda） | ✅ 実地確認済み |
> | Advanced Course2（Alarm → メール） | ✅ 実地確認済み（発火・復旧通知とも受信） |
> | ハマりポイント | ✅ 実地で遭遇した事象を反映済み |
>
> **開催前に確認すること**
>
> - AWS コンソールの画面名が変わっていないか（**2026 年に CloudWatch が刷新され「クラシックメトリクス」「ログ管理」「ログ分析」に変更された**）
> - 貸出 Raspberry Pi のタイムゾーンが `Asia/Tokyo` になっているか（初期値が `Europe/London` の機体がある）
>
> スクリーンショットは**挿入しない方針**です（画面表記の変更に追従できなくなるため、文章で記述する）。
...
`--target 70` は、**実行後のメモリ使用率を70%に近づける**指定です。現在の使用量を
差し引いた分だけを追加確保します（現在30%なら、総容量の約40%を追加確保）。

> ⚠️ 確保後のメモリ総使用量が**総容量の85%を超えないようにクランプ**されます
> （超える指定をすると警告が出ます）。Raspberry Pi が応答しなくなるのを防ぐためです。
...
> 💡 **途中で止まってしまっても、もう一度実行すれば続きから片付きます。** すでに消えているものは
> スキップされるので、何度実行しても問題ありません。

> 💡 「紐付く証明書は見つかりませんでした」と出た場合、**すでに削除済みなら問題ありません**。
> ただし証明書を発行したのに Thing へアタッチしていなかった場合はこの方法で検出できないため、
> IoT Core →「セキュリティ」→「証明書」で紐付けのない証明書が残っていないか確認してください。
...
> 💡 **CloudWatch のカスタムメトリクスには削除 API がありません。** 送信を止めてから
> 2 週間データポイントがないメトリクスは、コンソールや `list-metrics` の一覧に表示されなくなります。
> データは解像度を下げながら最長 15 か月保持され、その後自動的に期限切れになります。
> 課金対象の CloudWatch アラームはスタック削除で消えるので、後片付けをすれば月額課金は止まります。
>
> 参考：[CloudWatch のメトリクス保持期間](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/cloudwatch_concepts.html)
